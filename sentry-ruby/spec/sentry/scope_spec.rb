...
RSpec.describe Sentry::Scope do
  ...
  describe '#initialize' do
    it 'contains correct defaults' do
      ...
      expect(subject.contexts[:os].keys).to match_array(%i[name version build kernel_version machine])
  ...
  describe '#apply_to_event' do
    ...
    it 'applies the contextual data to event' do
      ...
      expect(event.contexts[:os].keys).to match_array(%i[name version build kernel_version machine])
    ...
    context 'with global event processor' do
      ...
      it 'scope event processors take precedence over global event processors' do
        subject.add_event_processor do |event, _hint|
          event.tags = { foo: 42 }
          event
        end
